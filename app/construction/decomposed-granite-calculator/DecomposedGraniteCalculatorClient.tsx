...
export default function DecomposedGraniteCalculatorClient() {
  ...
  const [pricePerTon, setPricePerTon] = useState("75");
  const [deliveryFee, setDeliveryFee] = useState("85");
  const [stabilizerCost, setStabilizerCost] = useState("0");
  ...
  const results = useMemo(() => {
    ...
    const totalCost =
      materialCost + toNumber(deliveryFee) + toNumber(stabilizerCost);
  ...
  async function copyResults() {
    ...
    const resultText = `Numeravo Decomposed Granite Estimate
Shape: ${shape}
${sizeText}
Depth: ${depth} in
Waste: ${wastePercent}%
...
Stabilizer / Add-On Cost: ${formatCurrency(toNumber(stabilizerCost))}
    ...
    <section className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
      <div className="rounded-2xl border border-[#1F2937] bg-[#121826] p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-3 text-sm leading-7 text-[#A0AEC0]">
              Calculate decomposed granite for patios, walkways, garden paths,
              landscape areas, dog runs, seating areas, and compacted surfaces.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <NumberInput label="Stabilizer / Add-On Cost" value={stabilizerCost} onChange={setStabilizerCost} prefix="$" />
        </div>
      ...
      <div className="rounded-2xl border border-[#1F2937] bg-[#121826] p-6">
        ...
        <div className="mt-6 rounded-2xl border border-[#F97316] bg-[#0B0F19] p-5">
          <p className="text-sm text-[#A0AEC0]">Decomposed granite needed</p>
        ...
        <div className="mt-6 space-y-4">
          ...
          <ResultRow label="Stabilizer / Add-On Cost" value={formatCurrency(toNumber(stabilizerCost))} />
        ...
        <div className="mt-6 rounded-xl border border-[#1F2937] bg-[#0B0F19] p-4">
          <p className="text-sm leading-6 text-[#A0AEC0]">
            Decomposed granite may be loose, stabilized, or resin-bound. Confirm
            density, compaction requirements, and stabilizer cost with your
            local supplier or installer.
          </p>
        </div>
      </div>
    </section>
