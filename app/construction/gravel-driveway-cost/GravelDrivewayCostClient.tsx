...
export default function GravelDrivewayCostClient() {
  ...
  const [spreadingCost, setSpreadingCost] = useState("250");
  ...
  const results = useMemo(() => {
    ...
    const spreading = toNumber(spreadingCost);
    ...
    const totalCost = materialCost + delivery + prep + spreading;
  ...
  async function copyResults() {
    const resultText = `Numeravo Gravel Driveway Cost Estimate
...
Spreading / Compaction Cost: ${formatCurrency(results.spreading)}
    ...
    <section className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
      <div className="rounded-2xl border border-[#1F2937] bg-[#121826] p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-semibold text-white">
              Estimate gravel driveway material, delivery, and total cost
            </h2>

            <p className="mt-3 text-sm leading-7 text-[#A0AEC0]">
              Enter driveway size, gravel depth, material price, delivery, prep,
              and spreading costs to estimate a more complete driveway project
              budget.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <NumberInput
            label="Spreading / Compaction"
            value={spreadingCost}
            onChange={setSpreadingCost}
            prefix="$"
          />
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#1F2937] bg-[#121826] p-6">
        ...
        <div className="mt-6 rounded-2xl border border-[#F97316] bg-[#0B0F19] p-5">
          <p className="text-sm text-[#A0AEC0]">Estimated total driveway cost</p>
          ...
          <p className="mt-2 text-sm text-[#A0AEC0]">
            Includes material, delivery, prep, and spreading/compaction.
          </p>
        </div>
        ...
        <div className="mt-5 space-y-4">
          ...
          <ResultRow
            label="Spreading / Compaction"
            value={formatCurrency(results.spreading)}
          />
        ...
        <div className="mt-6 rounded-xl border border-[#1F2937] bg-[#0B0F19] p-4">
          <p className="text-sm leading-6 text-[#A0AEC0]">
            This estimate is for planning only. Local gravel price, delivery
            distance, site access, grading, drainage, compaction, and supplier
            minimums can change the final project cost.
          </p>
        </div>
      </div>
    </section>
